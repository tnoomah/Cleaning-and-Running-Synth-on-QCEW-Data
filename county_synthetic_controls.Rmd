...
```{r}
select_counties_age <- 
  read_csv(url("https://ledextract.ces.census.gov/request/427f79bee0524db1853ff22584700dde.csv"))
...
select_counties_race <- 
  read_csv(url("https://ledextract.ces.census.gov/request/0f209b8fedfa4fc093aacbf6f6d9d304.csv"))
...
```{r}
select_counties_edu <- 
  read_csv(url("https://ledextract.ces.census.gov/request/995f57bf164c4f5389c26ddb6a58c4e3.csv"))
...
difs_fem_Emp_growth_44 <- 
  as.tibble(do.call(rbind, lapply(county_list, synthesizer, 0.44, "female_Emp_growth")))

difs_no_ba_earn_growth_44 %>% arrange(desc(annual_dependent_change))

difs_no_ba_Emp_growth_44 <- 
  as.tibble(do.call(rbind, lapply(county_list, synthesizer, 0.44, "no_ba_Emp_growth")))

difs_no_ba_earn_growth_44 %>% arrange(desc(annual_dependent_change))

difs_no_ba_earn_growth_44 <- 
  as.tibble(do.call(rbind, lapply(county_list, synthesizer, 0.44, "young_earnings_growth")))

difs_nonwhite_earn_growth_44 <- 
  as.tibble(do.call(rbind, lapply(county_list, synthesizer, 0.44, "nonwhite_earnings_growth")))

difs_latino_earn_growth_44 %>% arrange(desc(annual_dependent_change))
```